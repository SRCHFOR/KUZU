...
producerShowsIndex = new Index({
  name: 'producerShowsIndex',
  collection: Shows,
  fields: ['showName'],
  ...
  engine: new MongoDBEngine({
    selector(searchDefinition, options, aggregation) {
		...
		const searchString = searchDefinition.showName || ''
		var currentUser = Meteor.users.findOne({ _id: options.search.userId })

      	if (!currentUser) {
        	//options.search.userId = false
			//return options.search.userId
			//return { _id: false }
			selector.userId = false
        	return selector
      	}
		
		//if searchbar not empty return selector, else return custom query for all shows with dates
		if (searchString.trim() !== ''){
			const escapedString = searchString.trim().replace(/(\W{1})/g, '\\$1')
			return {
				$and: [{ 
					showName: {$regex: `.*${escapedString}.*`, $options: 'i' }},{
					$or: [{ userId: currentUser._id }, { helperUserId: currentUser._id }]
				}]
			}
		}
		else{
		//direct copy of showsWithDates helper
      	return {
        	$and: [{
        		$or:[{
      ...
      //data interceptor for console prints
      transform(doc) {
		//console.log('producerShowsIndex doc')
      	//console.log(doc)

		
		if (Meteor.isClient){
			if (/^[a-zA-Z0-9]{17}\"\"\{\}$/.test(doc._id)){
				//Reset "show more" button first time through after using search bar
				if (Session.get('usingSearchBar')){
					Session.set('showButtonDisabled', false)
				}
				Session.set('usingSearchBar', false)
			}
			else{
				//Reset "show more" button when first using search bar and save search criteria
				if (!Session.get('usingSearchBar')){
					Session.set('showButtonDisabled', false)
					Session.set('prevDoc_id', doc._id.slice(17))
				}
				//else reset "show more" button when search criteria first changes
				else {if(Session.get('prevDoc_id', doc._id) !== doc._id.slice(17)){
					Session.set('showButtonDisabled', false)
					Session.set('prevDoc_id', doc._id.slice(17))
				}}
				Session.set('usingSearchBar', true)
			}
		}
      
	  	//return the doc with full schema includeing helpers or nothing gets sent to the client
		return Shows._transform(doc)
		//return doc 
      }
  })
})
