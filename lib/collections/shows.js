...
Shows.helpers({
  ...
  tenMinutesPriorToShowTime() {
    var showStart = this.showStart.getTime()
    var timeNowPlusTen = new moment(new Date())
      .add(10, 'minutes')
      .valueOf()
  ...
  checkStartPressed(showId){
	...
	if (!show){
		...
		var showStart = show.showStart.getTime()
    	var timeNow = new Date().getTime()
		if (!!show.showStart && timeNow >= showStart){
			Shows.update(
      			{ _id: showId },
