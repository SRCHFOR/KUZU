	...
	Meteor.call('getAllUsers', function(error, result){
										if (!!error){
											...
											_.each(result, function(user) {
												//create array for email reminder subscription list
												if (('producerProfile' in user) && !!user.producerProfile.showName){
...
//This checks if new users has click verification email and then calls a method that sends an email to KUZU929 email alerting about the new user.
  if (Meteor.isServer && !!modifier.$set && modifier.$set['emails.$.verified'] && !modifier.$unset) {
